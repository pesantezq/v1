...
- Promotion-overlay durability (added 2026-07-28; `promotion_approvals.py` + `production_application.py`):
  - `durable_watchlist_op_ids` = the set of `candidate_id` (falling back to `proposal_id` for pre-Task-1 records) for every `WATCHLIST`-type op in the CURRENT `outputs/latest/approved_watchlist_proposals.json:ops`, deduplicated. `durable_watchlist_ops_live` = `len(durable_watchlist_op_ids)`. **This is the only valid count of live durable ops** — never derive it by counting `applied_to_production` rows in `production_application_audit.jsonl`, because that log gets one fresh row per surviving op on every run (it iterates ops *after* the durable union), so a naive row count grows daily for a single unchanged op and does not mean "N new applications."
  ...
  - `durability_regression_ids` = target ids that appear as an `applied_to_production` watchlist op at least once in `production_application_audit.jsonl`, are ABSENT from `durable_watchlist_op_ids` today, and are NOT in `revoked_ids` (an op vanished from the overlay with no human revocation on record — the exact defect this layer fixed).
  - `approvals_without_live_op` = valid entries in `promotion_approvals.effective_approvals_by_candidate(base_dir)` whose candidate/proposal id has produced no id in `durable_watchlist_op_ids` for > 2 runs (an approval that never took effect). `approvals_without_live_op_count` = its length.
  - `overlay_op_missing_valid_approval` = any id in `durable_watchlist_op_ids` with no matching valid record in `load_valid_approvals(base_dir)` (`is_human_approver` must pass) — a contract breach.
...
6s. Overlay durability (always when `production_application_audit.jsonl` exists; else `"Overlay-durability: inert (nothing applied yet)"`): `"Overlay-durability: {durable_watchlist_ops_live} durable watchlist op(s) live · {revoked_count} revoked · {approvals_without_live_op_count} approval(s) with no live op"` — the durable-overlay layer (added 2026-07-28; Tasks 1–4 of this branch). Watchlist ops are membership state and persist across runs until explicitly revoked; advisory ops refresh by design and are NOT expected to persist — never alert on an advisory op refreshing away. `durable_watchlist_ops_live` is read from the CURRENT overlay (`outputs/latest/approved_watchlist_proposals.json:ops`, deduped by `candidate_id`) — **never** from counting `applied_to_production` audit rows, since the audit write iterates ops *after* the durable union and a carried-forward op earns a fresh row on every run it survives; the audit log is used only to detect `durability_regression_ids` (see Step 1 Compute). **AMBER** when `durability_regression_ids` is non-empty (a previously-applied watchlist op vanished from the overlay with no matching row in `production_revocations.jsonl` — the exact defect this layer fixed), OR `approvals_without_live_op_count ≥ 1` (a valid human approval that never produced a live op). **RED** only on a contract breach: `overlay_op_missing_valid_approval` (a live overlay op with no matching valid human approval per `is_human_approver`), or `revoked_id_still_in_overlay` (a revoked target still present in the overlay). Source: `production_application_audit.jsonl` (regression detection only — not a count) + `production_revocations.jsonl` (note the `.jsonl` extension, changed from `.json` during Task 4's fix round) + `approved_proposals.json` + `outputs/latest/approved_watchlist_proposals.json` (source of truth for the live count).
