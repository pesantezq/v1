...
6g. Operator-control (always): `"Operator-control: {open} open · {awaiting_approval} awaiting · {failed} failed · {quarantined} quarantined · {stale} stale · worker {mode} · readiness {overall_ready} · cost today ${today_usd}{/{cap_usd} ({cap_pct}% of daily) if cap_configured else ' (uncapped)'} · lifetime ${lifetime_usd}{ · deferred {n} · over-cap {m} if any cost-cap events today}"` — observe-only; computed live from `operator_worker_view` (NO new persisted artifact). AMBER on `quarantined ≥ 1`, stale-run, `cap_pct ≥ 80` (daily cost cap nearly exhausted), or any `worker_cost_cap_exceeded` event today; readiness `<5/5` is reported, not alerted (it's the hardening-milestone tracker). Never RED.
...
6n. Sim-governance (always): `"Sim-gov: lane {active|disabled} · {sim_gov_candidates} cand. · AI {sim_gov_ai_status} ${sim_gov_ai_cost}/{sim_gov_ai_cap} cap · {sim_gov_pending} pending · {sim_gov_applied} applied · overlays {live|off}"` — the two-lane simulation-governance lane (Stage 10e). Simulation lane ACTIVE; production promotion human-gated; overlays default-OFF. AMBER on `sim_gov_ai_deferred`, `sim_gov_pending_backlog`, or an applied proposal lacking a valid human approval. **Never RED** (the lane never feeds `decision_plan.json`; production overlays apply only human-approved, audited changes). Absent artifacts = inert (not yet run), report don't alert. Source: `daily_governance_status.json` + `daily_ai_review_result.json` + `pending_proposals.json` + `production_application_state.json`.
