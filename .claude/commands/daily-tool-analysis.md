...
29. Promotion-overlay durability (`portfolio_automation/sim_governance/promotion_approvals.py` + `production_application.py`, added 2026-07-28 — durable human approvals + human-gated revoke path over the production overlay). Read (degrade gracefully on any miss):
    - `outputs/latest/approved_watchlist_proposals.json:ops` — the CURRENT production watchlist overlay; this is the source of truth for what is live today.
    - `outputs/promotion_approvals/production_application_audit.jsonl` (append-only) — history of `applied_to_production` events. **Do not count rows as a live-application count**: the audit write iterates ops *after* the durable union, so a carried-forward op gets a fresh `applied_to_production` row on every run it survives, not only when newly applied. Use this log only to detect a *regression* (see Compute below), never to count applications.
    - `outputs/promotion_approvals/production_revocations.jsonl` (append-only JSONL, one record per line — **note the `.jsonl` extension**; this is a Task-4 fix, not `.json`) via `promotion_approvals.revoked_ids(base_dir)`.
    - `outputs/promotion_approvals/approved_proposals.json` via `promotion_approvals.effective_approvals_by_candidate(base_dir)` / `load_valid_approvals(base_dir)`.
    - `outputs/promotion_approvals/production_application_state.json` → `overlay_rebuild_skipped` + `approvals_log_unreadable` (fail-closed refusal signal), and `watchlist_applied_today` / `watchlist_carried_forward` / `durably_live_count` (so "newly applied today" is never confused with "durably live" — `applied_count` counts only today's loop).
...
- Promotion-overlay durability (added 2026-07-28; `promotion_approvals.py` + `production_application.py`):
  - **Durable proposal types** = exactly `production_application._DURABLE_PROPOSAL_TYPES` (`watchlist_add`, `watchlist_remove`, `watchlist_rank_change`, `watchlist_tag_change`) — use the helper `production_application.is_durable_proposal_type(ptype)`, never `workflow_for_proposal_type(...) == WORKFLOW_WATCHLIST`. Durability follows the proposal TYPE, not the workflow (operator decision 2026-07-28): membership *decisions* persist; state-derived *labels* (`flock_watchlist_candidate_logic`, every advisory type) refresh by design and must NEVER be counted as durable or alerted on for refreshing away.
  - `durable_watchlist_op_ids` = for every op in the CURRENT `outputs/latest/approved_watchlist_proposals.json:ops` whose `proposal_type` is a durable type, the set of BOTH its `candidate_id` and its `proposal_id` (whichever are present), deduplicated. `durable_watchlist_ops_live` = the number of such **ops** (not ids — a new op contributes two ids). **This is the only valid count of live durable ops** — never derive it by counting `applied_to_production` rows in `production_application_audit.jsonl`, because that log gets one fresh row per surviving op on every run (it iterates ops *after* the durable union), so a naive row count grows daily for a single unchanged op and does not mean "N new applications."
  - `revoked_ids` = `promotion_approvals.revoked_ids(base_dir)` over `production_revocations.jsonl` (append-only JSONL, one record per line — note the `.jsonl` extension, changed from `.json` during Task 4's fix round). `revoked_count` = `len(revoked_ids)`.
  - `rolled_back_ids` = the union of `rolled_back_ids` on every `rolled_back` event in `production_application_audit.jsonl` for `approved_watchlist_proposals.json` (written by `production_application.rollback_last`). A rolled-back op is *expected* to be absent from the overlay — treat it exactly like a revocation when computing `durability_regression_ids`.
  - `durability_regression_ids` = target ids that appear as an `applied_to_production` op **of a durable type** at least once in `production_application_audit.jsonl`, are ABSENT from `durable_watchlist_op_ids` today, and are in NEITHER `revoked_ids` NOR `rolled_back_ids` (an op vanished from the overlay with no human revocation or rollback on record — the exact defect this layer fixed). Ids from non-durable types (all 17 pre-2026-07-28 rows are `flock_watchlist_candidate_logic` / advisory) are excluded by construction, so they must never appear here.
  - `approvals_without_live_op` = valid entries in `promotion_approvals.effective_approvals_by_candidate(base_dir)` **whose approved proposal is of a durable type** (resolve the type via `pending_proposals.json` / `proposals_log.jsonl`; skip any approval whose type cannot be resolved) that have produced no matching id in `durable_watchlist_op_ids` for > 2 runs — an approval that never took effect. `approvals_without_live_op_count` = its length. The durable-type filter is load-bearing: advisory approvals produce no durable op *by design*, so without it this signal counts every advisory approval and can never clear.
  - `overlay_op_missing_valid_approval` = any live durable op for which NEITHER its `candidate_id` NOR its `proposal_id` matches a valid record in `load_valid_approvals(base_dir)` (`is_human_approver` must pass) — a contract breach. Match on **either** id: a new durable op is identified by `candidate_id` while legacy approvals carry only `proposal_id`, so comparing one keyspace against the other would fire RED on the first healthy new op.
  - `revoked_id_still_in_overlay` = any id in `revoked_ids` that is still present in `durable_watchlist_op_ids` — a contract breach (a revocation did not actually remove the op).
  - `overlay_rebuild_skipped` = `outputs/promotion_approvals/production_application_state.json:overlay_rebuild_skipped` (with `approvals_log_unreadable` naming the reason). True means `apply_approved_proposals` FAILED CLOSED: `approved_proposals.json` exists but could not be parsed, so the rebuild was refused and the overlay on disk is whatever the last good run wrote. The refusal is the control working; the unreadable approvals log is the problem.
...
6s. Overlay durability (always when `production_application_audit.jsonl` exists; else `"Overlay-durability: inert (nothing applied yet)"`): `"Overlay-durability: {durable_watchlist_ops_live} durable op(s) live ({watchlist_applied_today} new today) · {revoked_count} revoked · {approvals_without_live_op_count} durable approval(s) with no live op"` — the durable-overlay layer (added 2026-07-28). Durability is a property of the proposal TYPE, not the workflow: only `_DURABLE_PROPOSAL_TYPES` (`watchlist_add` / `watchlist_remove` / `watchlist_rank_change` / `watchlist_tag_change`) are membership decisions that persist until explicitly revoked. `flock_watchlist_candidate_logic` and every advisory type are state-derived labels that refresh by design — never count them as durable and never alert on one refreshing away. `durable_watchlist_ops_live` is read from the CURRENT overlay (`outputs/latest/approved_watchlist_proposals.json:ops`, durable types only) — **never** from counting `applied_to_production` audit rows, since the audit write iterates ops *after* the durable union and a carried-forward op earns a fresh row on every run it survives; the audit log is used only to detect `durability_regression_ids` (see Step 1 Compute). `watchlist_applied_today` comes from `production_application_state.json` and distinguishes "newly applied today" from "durably live" — `applied_count` alone reads 0 on a quiet day while ops are still in force, so never present it as the live figure. **AMBER** when `durability_regression_ids` is non-empty (a previously-applied durable op vanished from the overlay with no matching revocation or rollback on record — the exact defect this layer fixed), OR `approvals_without_live_op_count ≥ 1` (a valid human approval **of a durable type** that never produced a live op; advisory approvals are excluded by construction, so this signal must be able to reach 0). **RED** only on a contract breach: `overlay_op_missing_valid_approval` (a live durable op whose `candidate_id` AND `proposal_id` both fail to match any valid human approval per `is_human_approver` — match on **either** id, since new ops are identified by `candidate_id` while legacy approvals carry only `proposal_id`), `revoked_id_still_in_overlay` (a revoked target still present in the overlay), or `overlay_rebuild_skipped` true (the rebuild fail-closed because `approved_proposals.json` is unreadable — the refusal is correct, the unreadable approval authority is the RED). **On RED, dispatch `portfolio-learning-loop-health`** (process-analyst lens, same owner as 6n3 / 6q) with the offending ids, the state file's `approvals_log_unreadable` reason if set, and the instruction to VERIFY against the approval records + audit trail rather than to revert the overlay. Source: `production_application_audit.jsonl` (regression/rollback detection only — not a count) + `production_revocations.jsonl` (note the `.jsonl` extension, changed from `.json` during Task 4's fix round) + `approved_proposals.json` + `production_application_state.json` + `outputs/latest/approved_watchlist_proposals.json` (source of truth for the live count).
