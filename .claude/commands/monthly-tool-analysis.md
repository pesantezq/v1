...
20. Research-Backed Strategy Lab (added 2026-06-12; weekly; **observe-only, sandbox-only, quant lens**). Run the deterministic assessor and fold its verdict in:
    ...
    Reads `outputs/sandbox/{strategy_leaderboard, research_strategy_catalog, walk_forward_results, factor_exposure_report}.json`. **WS4 (2026-07-28):** the verdict is now a fail-closed roll-up of 9 dimensions (`result["dimensions"]`, `result["blocking_reasons"]`) gated ON by default (`portfolio_sim.strategy_lab.health.strict_oos_rollup_enabled`; legacy keys still resolve). RED = `looks_fresh_but_empty` (lab ran but every tactic degraded — check archive coverage) or an invalid `created_at`. AMBER = disabled/insufficient/stale/undocumented_tactics/any `OOS_FAILED` tactic/`factor_data_unavailable`/**zero tactics reaching `OOS_SUPPORTED`** (the fixed headline defect — "no failing-OOS tactic surfaced" no longer reads as GREEN)/`<50%` tactics with sufficient-fold OOS evidence/top-ranked tactic not `OOS_SUPPORTED`. GREEN = ran, populated, documented, no `OOS_FAILED` tactic, AND ≥1 tactic reaches `OOS_SUPPORTED`. As of 2026-07-28 the real leaderboard is AMBER (25/26 tactics `OOS_NOT_TESTED`; the 1 tested tactic is ranked last) — this is intended, not a regression; do not re-flag it monthly as new. The dedicated `/strategy-lab-analysis` skill is the on-demand equivalent. Never RED-blocks the decision core (research lane, never feeds `decision_plan`).
