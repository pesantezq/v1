...
def build_adjusted_bars(provider: Any, symbols: list[str], *,
    ...
    # universe, then keeps only the eligible symbols), a short-history symbol
    # that will be excluded anyway must not fail the build with a gap — so gap
...
def gap_check_kept(panel: dict[str, list[C.BarRow]], kept: set[str]) -> None:
    """Refuse a kept symbol with a benchmark-session gap wider than the frozen
    contract allows. Fail closed; forward-filling across it would invent
    sessions."""
    if C.BENCHMARK not in panel:
        return
    bench_dates = [b.session_date for b in panel[C.BENCHMARK]]
    for sym in sorted(kept):
        if sym == C.BENCHMARK or sym not in panel:
            continue
...
def adjusted_return_panel(panel: dict[str, list[C.BarRow]], kept: set[str],
                          start_date: str, end_date: str) -> list[C.ReturnRow]:
    """The authoritative VS-002 risk-return series, derived from the ADJUSTED
    bars: r(t) = adj_close(t)/adj_close(t-1) - 1. This is the single return
    surface in adjusted-bar mode; the legacy archive-close panel is not built
    and cannot compete with it."""
    out: list[C.ReturnRow] = []
    for sym in sorted(kept):
        bars = panel.get(sym, [])
        for i in range(1, len(bars)):
            d = bars[i].session_date
            if d < start_date or d > end_date:
                continue
            prev = bars[i - 1]
            out.append(C.ReturnRow(
                symbol=sym, session_date=d, prev_session_date=prev.session_date,
                close=bars[i].adj_close, prev_close=prev.adj_close,
                daily_return=C.derive_adjusted_return(
                    bars[i].adj_close, prev.adj_close)))
    return out
...
def full_panel_from_raw(raw_responses: dict[str, list],
                        symbols: set[str]) -> dict[str, list[C.BarRow]]:
    """Normalize the ENTIRE frozen provider response for each symbol (no
    windowing), through the same normalization the builder used. This is the
    panel the universal >=252-prior-session rule is applied to; the consumer
    reuses it to recompute bar eligibility from frozen bytes alone."""
    return {sym: _normalize_bars(sym, raw_responses[sym])
            for sym in symbols if sym in raw_responses}
...
def build(repo_root: Path, *, db_rel: str = DEFAULT_DB_REL,
    ...
    adjusted_mode = bar_provider is not None

    # In adjusted-bar mode the dividend-adjusted panel is the AUTHORITATIVE
    # VS-002 risk evidence: it owns symbol eligibility, signal inclusion,
    # cohorts and the return series. The legacy /full archive can neither veto
    # a symbol the adjusted evidence would keep nor rescue one it would drop,
    # so it is not even loaded here. In legacy (no-bar) mode the historical
    ...
    if not adjusted_mode:
        # ---------- LEGACY archive-close path (unchanged) ------------------
        ...
        risk_return_source = "legacy_archive_close"
        return_derivation = C.BETA_FORMULA
        return_beta_convention = C.BETA_CONVENTION
    else:
        # ---------- AUTHORITATIVE dividend-adjusted path -------------------
        # Eligibility is decided by the adjusted panel, from the FULL matured
        # signal set — the legacy archive is never consulted, so it cannot
        # remove a signal before the adjusted evidence evaluates it.
        ...
        gap_check_kept(panel, kept_bars)

        # Signal inclusion follows ADJUSTED-bar eligibility, not the archive.
        signals = [s for s in signals if s.ticker in kept_bars]
        signal_dates = sorted({s.signal_time[:10] for s in signals})
        cohorts = greedy_cohorts(signal_dates)

        bench_bar_dates = [b.session_date for b in panel[C.BENCHMARK]]
        first_signal_date = min(earliest_all[s] for s in earliest_all
                                if s in bar_eligible)
        ...
        # The authoritative return series is DERIVED FROM THE ADJUSTED BARS,
        # so there is exactly one risk-return surface and it is the adjusted
        # one. returns.json carries it; the executor cannot select a legacy
        # archive-close series because none is built.
        returns = adjusted_return_panel(panel, kept_bars, bar_start, cutoff_date)
        if not returns:
            raise BuildError("empty adjusted return panel")

        eligible, excluded = bar_eligible, bar_excluded
        kept = kept_bars
        start_date = bar_start
        risk_return_source = "dividend_adjusted_bars"
        return_derivation = C.ADJUSTED_BETA_FORMULA
        return_beta_convention = C.ADJUSTED_BETA_CONVENTION

        # Finding 2: freeze raw provider input + retrieval time for EVERY
        # fetched frozen symbol, not merely the kept ones, so an
        # excluded-symbol decision is independently replayable by the lab.
        fetched = sorted(set(raw_responses))
        bars_raw_payload = {sym: list(raw_responses[sym]) for sym in fetched}
        bar_retrieved_at = {
            sym: retrieved_at[sym].astimezone(timezone.utc).strftime(
                "%Y-%m-%dT%H:%M:%SZ")
            for sym in fetched if sym in retrieved_at}
        # Canonical snapshots: only the CONSUMED (kept) bars are calculation
        # evidence and get EvidenceSnapshots/Refs; the full-universe raw above
        # is eligibility-supporting evidence, distinguished here on purpose.
        ...
        bar_manifest = {
            ...
            # Raw digests + retrieval span the WHOLE fetched universe so the
            # exclusion arithmetic is reproducible; snapshots are kept-only.
            "bar_raw_response_digests": {
                k: raw_digests[k] for k in fetched},
            "bar_retrieved_at": bar_retrieved_at,
            "bar_fetched_universe": fetched,
            "bar_earliest_signal": {s: earliest_all[s]
                                    for s in sorted(earliest_all)},
    ...
    manifest_core = {
        ...
        "return_derivation": return_derivation,
        "beta_convention": return_beta_convention,
