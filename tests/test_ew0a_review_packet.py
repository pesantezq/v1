...
def _bound():
    """A candidate binding that has already been satisfied.

    These tests exercise the COMPLETENESS gate, so the candidate binding is
    supplied pre-satisfied rather than re-proven here; the binding itself is
    tested against real commits in test_review_candidate_binding.py. It is
    constructed explicitly on every call because dispatch_review takes it as a
    required argument with no default -- there is deliberately no value that
    means "skip the candidate check"."""
    return CandidateBinding(packet_sha=SHA, head_at_binding=SHA, repo=None,
                            checks={"REVIEW_CANDIDATE_SHA_EQUALS_GIT_HEAD": "YES"})
...
def test_a_missing_test_source_blocks_dispatch():
    ...
    out = dispatch_review(p, spy, candidate=_bound(), screen=False)
...
def test_e2_no_criteria_at_all_fails_closed():
    spy = ReviewerSpy()
    out = dispatch_review(_packet(), spy, candidate=_bound(), screen=False)
    assert out.dispatched is False and spy.was_called is False
...
def test_f_candidate_sha_mismatch_fails_closed():
    ...
    p.add_evidence(_result())
    p.bind("C1_TAMPER", "tests/test_evidence_gateway_admissibility.py", "focused-tests")
    out = dispatch_review(p, spy, expected_sha=SHA, candidate=_bound(), screen=False)
    assert out.dispatched is False and spy.was_called is False
...
def test_h_secret_screen_refusal_blocks_dispatch():
    ...
    p.add_evidence(_result())
    p.bind("C1_TAMPER", "tests/test_evidence_gateway_admissibility.py", "focused-tests")
    out = dispatch_review(p, spy, candidate=_bound(), screen=True)
    assert out.dispatched is False and spy.was_called is False
...
def test_multiple_criteria_all_must_be_satisfied():
    ...
    p.bind("C1_TAMPER", "tests/test_evidence_gateway_admissibility.py", "focused-tests")
    p.bind("C2_GUI")
    out = dispatch_review(p, spy, candidate=_bound(), screen=False)      # GUI evidence absent
    assert out.dispatched is False and spy.was_called is False
    assert "C1_TAMPER" in out.completeness.satisfied
