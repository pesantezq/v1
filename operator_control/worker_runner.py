...
def _run_via_container(worktree_path, prompt_md: str, mode: str, cfg: dict, root: str,
    ...
    try:
        ...
        # Run the container AS the rootless worker user with its own runtime dir +
        # session bus (else it inherits root's XDG_RUNTIME_DIR=/run/user/0 and
        # rootless crun fails). Shared with the capability probe so both target the
        # same context. Returns [] (run in-process) if already the worker user.
        argv = [*worker_container.worker_cmd_prefix(cfg), *spec]
